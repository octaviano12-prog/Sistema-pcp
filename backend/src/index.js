...
app.post('/api/auth/login', (req, res) => {
  ...
  const company = user.company_id ? getDb().prepare('SELECT * FROM companies WHERE id = ?').get(user.company_id) : null;
...
app.get('/api/auth/me', authenticateToken, (req, res) => {
  const user = getDb().prepare('SELECT id, name, email, role, company_id, status, created_at FROM users WHERE id = ?').get(req.user.id);
  if (!user) return res.status(404).json({ error: 'Usuário não encontrado' });
  const company = user.company_id ? getDb().prepare('SELECT id, name, trade_name, plan, status FROM companies WHERE id = ?').get(user.company_id) : null;
  res.json({ ...user, company });
});

// ==================== COMPANIES ====================
app.get('/api/companies', authenticateToken, requireRole('super_admin'), (req, res) => {
  const companies = getDb().prepare('SELECT * FROM companies ORDER BY created_at DESC').all();
  res.json(companies);
});

app.post('/api/companies', authenticateToken, requireRole('super_admin'), (req, res) => {
  const { name, trade_name, cnpj, email, phone, plan } = req.body;
  const result = getDb().prepare('INSERT INTO companies (name, trade_name, cnpj, email, phone, plan) VALUES (?, ?, ?, ?, ?, ?)').run(name, trade_name, cnpj, email, phone, plan || 'starter');
  res.json({ id: result.lastInsertRowid, ...req.body });
});

app.put('/api/companies/:id', authenticateToken, requireRole('super_admin'), (req, res) => {
  const { name, trade_name, cnpj, email, phone, plan, status } = req.body;
  getDb().prepare('UPDATE companies SET name=?, trade_name=?, cnpj=?, email=?, phone=?, plan=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(name, trade_name, cnpj, email, phone, plan, status, req.params.id);
  res.json({ success: true });
});

app.delete('/api/companies/:id', authenticateToken, requireRole('super_admin'), (req, res) => {
...
app.get('/api/users', authenticateToken, (req, res) => {
  ...
  const users = getDb().prepare('SELECT id, name, email, role, status, created_at FROM users WHERE company_id = ?').all(filter.company_id);
  res.json(users);
});

app.post('/api/users', authenticateToken, requireRole('super_admin', 'admin'), (req, res) => {
  ...
  try {
    const result = getDb().prepare('INSERT INTO users (company_id, name, email, password_hash, role) VALUES (?, ?, ?, ?, ?)').run(company_id, name, email, password_hash, role || 'viewer');
...
app.put('/api/users/:id', authenticateToken, (req, res) => {
  ...
  getDb().prepare('UPDATE users SET name=?, email=?, role=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(name, email, role, status, req.params.id);
  res.json({ success: true });
});

app.delete('/api/users/:id', authenticateToken, requireRole('super_admin', 'admin'), (req, res) => {
...
app.get('/api/customers', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const customers = getDb().prepare('SELECT * FROM customers WHERE company_id = ? ORDER BY name').all(filter.company_id);
  res.json(customers);
});
...
app.post('/api/customers', authenticateToken, (req, res) => {
  ...
  const result = getDb().prepare('INSERT INTO customers (company_id, name, document, email, phone, whatsapp, address, city, state, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, name, document, email, phone, whatsapp, address, city, state, notes);
  res.json({ id: result.lastInsertRowid, ...req.body });
});

app.put('/api/customers/:id', authenticateToken, (req, res) => {
  const { name, document, email, phone, whatsapp, address, city, state, notes, active } = req.body;
  getDb().prepare('UPDATE customers SET name=?, document=?, email=?, phone=?, whatsapp=?, address=?, city=?, state=?, notes=?, active=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(name, document, email, phone, whatsapp, address, city, state, notes, active, req.params.id);
  res.json({ success: true });
});
...
app.get('/api/products/:id', authenticateToken, (req, res) => {
  const product = getDb().prepare('SELECT * FROM products WHERE id = ? AND company_id = ?').get(req.params.id, req.user.company_id);
...
app.put('/api/products/:id', authenticateToken, (req, res) => {
  ...
  getDb().prepare('UPDATE products SET code=?, name=?, description=?, type=?, unit=?, cost_price=?, sale_price=?, min_stock=?, max_stock=?, lead_time_days=?, weight=?, dimensions=?, active=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?').run(code, name, description, type, unit, cost_price, sale_price, min_stock, max_stock, lead_time_days, weight, dimensions, active, req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.delete('/api/products/:id', authenticateToken, (req, res) => {
  getDb().prepare('DELETE FROM products WHERE id = ? AND company_id = ?').run(req.params.id, req.user.company_id);
  res.json({ success: true });
});

// ==================== BOM (FICHA TÉCNICA) ====================
app.get('/api/boms', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const boms = getDb().prepare(`SELECT pb.*, p.name as component_name, p.code as component_code, p.unit as component_unit, p.cost_price as component_cost
...
app.get('/api/boms/product/:productId', authenticateToken, (req, res) => {
  const boms = getDb().prepare(`SELECT pb.*, p.name as component_name, p.code as component_code, p.unit as component_unit, p.cost_price as component_cost
...
app.post('/api/boms', authenticateToken, (req, res) => {
  const company_id = req.user.company_id;
  ...
  const result = getDb().prepare('INSERT INTO product_boms (company_id, product_id, component_id, quantity, unit, loss_percentage, unit_cost, total_cost, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, product_id, component_id, quantity, unit, loss_percentage || 0, unit_cost || 0, total_cost, notes);
  res.json({ id: result.lastInsertRowid, ...req.body, total_cost });
});

app.put('/api/boms/:id', authenticateToken, (req, res) => {
  const { quantity, unit, loss_percentage, unit_cost, notes } = req.body;
  const total_cost = (quantity || 0) * (unit_cost || 0) * (1 + (loss_percentage || 0) / 100);
  getDb().prepare('UPDATE product_boms SET quantity=?, unit=?, loss_percentage=?, unit_cost=?, total_cost=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?').run(quantity, unit, loss_percentage, unit_cost, total_cost, notes, req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.delete('/api/boms/:id', authenticateToken, (req, res) => {
  getDb().prepare('DELETE FROM product_boms WHERE id = ? AND company_id = ?').run(req.params.id, req.user.company_id);
  res.json({ success: true });
});
...
app.get('/api/routes', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const routes = getDb().prepare(`SELECT pr.*, m.name as machine_name FROM production_routes pr LEFT JOIN machines m ON pr.machine_id = m.id WHERE pr.company_id = ? ORDER BY pr.product_id, pr.sequence`).all(filter.company_id);
  res.json(routes);
});

app.get('/api/routes/product/:productId', authenticateToken, (req, res) => {
  const routes = getDb().prepare(`SELECT pr.*, m.name as machine_name FROM production_routes pr LEFT JOIN machines m ON pr.machine_id = m.id WHERE pr.product_id = ? AND pr.company_id = ? ORDER BY pr.sequence`).all(req.params.productId, req.user.company_id);
  res.json(routes);
});
...
app.post('/api/routes', authenticateToken, (req, res) => {
  ...
  const result = getDb().prepare('INSERT INTO production_routes (company_id, product_id, sequence, operation_name, machine_id, standard_time_minutes, setup_time_minutes, hourly_cost, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, product_id, sequence, operation_name, machine_id, standard_time_minutes || 0, setup_time_minutes || 0, hourly_cost || 0, notes);
  res.json({ id: result.lastInsertRowid, ...req.body });
});
...
app.put('/api/routes/:id', authenticateToken, (req, res) => {
  ...
  getDb().prepare('UPDATE production_routes SET sequence=?, operation_name=?, machine_id=?, standard_time_minutes=?, setup_time_minutes=?, hourly_cost=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?').run(sequence, operation_name, machine_id, standard_time_minutes, setup_time_minutes, hourly_cost, notes, req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.delete('/api/routes/:id', authenticateToken, (req, res) => {
  getDb().prepare('DELETE FROM production_routes WHERE id = ? AND company_id = ?').run(req.params.id, req.user.company_id);
  res.json({ success: true });
});
...
app.get('/api/machines', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const machines = getDb().prepare('SELECT * FROM machines WHERE company_id = ? ORDER BY name').all(filter.company_id);
  res.json(machines);
});
...
app.post('/api/machines', authenticateToken, (req, res) => {
  ...
  const result = getDb().prepare('INSERT INTO machines (company_id, name, code, type, sector, capacity_per_hour, hourly_cost, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, name, code, type, sector, capacity_per_hour || 0, hourly_cost || 0, notes);
  res.json({ id: result.lastInsertRowid, ...req.body });
});
...
app.put('/api/machines/:id', authenticateToken, (req, res) => {
  ...
  getDb().prepare('UPDATE machines SET name=?, code=?, type=?, sector=?, capacity_per_hour=?, hourly_cost=?, status=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?').run(name, code, type, sector, capacity_per_hour, hourly_cost, status, notes, req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.delete('/api/machines/:id', authenticateToken, (req, res) => {
  getDb().prepare('DELETE FROM machines WHERE id = ? AND company_id = ?').run(req.params.id, req.user.company_id);
  res.json({ success: true });
});

// ==================== STOCK ====================
function getStockForProduct(company_id, product_id) {
  const movements = getDb().prepare('SELECT * FROM stock_movements WHERE company_id = ? AND product_id = ? ORDER BY created_at').all(company_id, product_id);
  let stock = 0;
  movements.forEach(m => {
...
app.get('/api/stock', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const products = getDb().prepare('SELECT * FROM products WHERE company_id = ? AND active = 1 ORDER BY name').all(filter.company_id);
  const stock = products.map(p => {
    const current = getStockForProduct(filter.company_id, p.id);
    const reserved = getDb().prepare('SELECT COALESCE(SUM(reserved_quantity - consumed_quantity), 0) as reserved FROM production_order_materials WHERE company_id = ? AND product_id = ? AND status IN (?, ?)').get(filter.company_id, p.id, 'reserved', 'in_progress').reserved;
...
app.get('/api/stock/product/:productId', authenticateToken, (req, res) => {
  const movements = getDb().prepare('SELECT sm.*, u.name as user_name, p.name as product_name FROM stock_movements sm LEFT JOIN users u ON sm.created_by = u.id JOIN products p ON sm.product_id = p.id WHERE sm.product_id = ? AND sm.company_id = ? ORDER BY sm.created_at DESC').all(req.params.productId, req.user.company_id);
  res.json(movements);
});

app.post('/api/stock/movement', authenticateToken, (req, res) => {
  const company_id = req.user.company_id;
  ...
  const result = getDb().prepare('INSERT INTO stock_movements (company_id, product_id, type, quantity, unit_cost, total_cost, reason, reference_type, reference_id, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, product_id, type, quantity, unit_cost || 0, total_cost, reason, reference_type, reference_id, req.user.id);
  res.json({ id: result.lastInsertRowid, ...req.body });
});

app.get('/api/stock/movements', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const movements = getDb().prepare('SELECT sm.*, p.name as product_name, p.code as product_code, u.name as user_name FROM stock_movements sm JOIN products p ON sm.product_id = p.id LEFT JOIN users u ON sm.created_by = u.id WHERE sm.company_id = ? ORDER BY sm.created_at DESC LIMIT 200').all(filter.company_id);
  res.json(movements);
});
...
app.get('/api/sales-orders', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const orders = getDb().prepare(`SELECT so.*, c.name as customer_name FROM sales_orders so JOIN customers c ON so.customer_id = c.id WHERE so.company_id = ? ORDER BY so.created_at DESC`).all(filter.company_id);
  const items = getDb().prepare(`SELECT soi.*, p.name as product_name, p.code as product_code FROM sales_order_items soi JOIN products p ON soi.product_id = p.id WHERE soi.company_id = ?`).all(filter.company_id);
...
app.get('/api/sales-orders/:id', authenticateToken, (req, res) => {
  const order = getDb().prepare('SELECT so.*, c.name as customer_name FROM sales_orders so JOIN customers c ON so.customer_id = c.id WHERE so.id = ? AND so.company_id = ?').get(req.params.id, req.user.company_id);
  if (!order) return res.status(404).json({ error: 'Pedido não encontrado' });
  const items = getDb().prepare('SELECT soi.*, p.name as product_name, p.code as product_code FROM sales_order_items soi JOIN products p ON soi.product_id = p.id WHERE soi.sales_order_id = ?').all(req.params.id);
...
app.post('/api/sales-orders', authenticateToken, (req, res) => {
  ...
  const lastOrder = getDb().prepare('SELECT order_number FROM sales_orders WHERE company_id = ? ORDER BY id DESC LIMIT 1').get(company_id);
  const nextNum = lastOrder ? parseInt(lastOrder.order_number.replace('PV-', '')) + 1 : 1;
  const order_number = `PV-${String(nextNum).padStart(4, '0')}`;

  const result = getDb().prepare('INSERT INTO sales_orders (company_id, customer_id, order_number, order_date, delivery_date, priority, notes) VALUES (?, ?, ?, ?, ?, ?, ?)').run(company_id, customer_id, order_number, order_date || new Date().toISOString().split('T')[0], delivery_date, priority || 'normal', notes);
  const orderId = result.lastInsertRowid;

  if (items && items.length > 0) {
    items.forEach(item => {
      const total_price = (item.quantity || 0) * (item.unit_price || 0);
      getDb().prepare('INSERT INTO sales_order_items (company_id, sales_order_id, product_id, quantity, unit_price, total_price) VALUES (?, ?, ?, ?, ?, ?)').run(company_id, orderId, item.product_id, item.quantity, item.unit_price, total_price);
    });
  }
...
app.put('/api/sales-orders/:id', authenticateToken, (req, res) => {
  ...
  getDb().prepare('UPDATE sales_orders SET customer_id=?, delivery_date=?, priority=?, status=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?').run(customer_id, delivery_date, priority, status, notes, req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.delete('/api/sales-orders/:id', authenticateToken, (req, res) => {
  getDb().prepare('DELETE FROM sales_order_items WHERE sales_order_id = ?').run(req.params.id);
  getDb().prepare('DELETE FROM sales_orders WHERE id = ? AND company_id = ?').run(req.params.id, req.user.company_id);
  res.json({ success: true });
});

// ==================== PRODUCTION ORDERS ====================
app.get('/api/production-orders', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const orders = getDb().prepare(`SELECT po.*, p.name as product_name, p.code as product_code, c.name as customer_name, so.order_number as sales_order_number
...
app.get('/api/production-orders/:id', authenticateToken, (req, res) => {
  const order = getDb().prepare(`SELECT po.*, p.name as product_name, p.code as product_code, c.name as customer_name
  ...
  const materials = getDb().prepare('SELECT pom.*, p.name as product_name, p.code as product_code FROM production_order_materials pom JOIN products p ON pom.product_id = p.id WHERE pom.production_order_id = ?').all(req.params.id);
  const operations = getDb().prepare('SELECT poo.*, m.name as machine_name FROM production_order_operations poo LEFT JOIN machines m ON poo.machine_id = m.id WHERE poo.production_order_id = ? ORDER BY poo.sequence').all(req.params.id);
  const logs = getDb().prepare('SELECT pl.*, u.name as user_name, m.name as machine_name FROM production_logs pl LEFT JOIN users u ON pl.user_id = u.id LEFT JOIN machines m ON pl.machine_id = m.id WHERE pl.production_order_id = ? ORDER BY pl.created_at DESC').all(req.params.id);

  res.json({ ...order, materials, operations, logs });
});

app.post('/api/production-orders', authenticateToken, (req, res) => {
  const company_id = req.user.company_id;
  const { product_id, sales_order_id, customer_id, planned_quantity, planned_start_date, planned_end_date, priority, notes } = req.body;

  // Check BOM exists
  const bomCount = getDb().prepare('SELECT COUNT(*) as count FROM product_boms WHERE product_id = ? AND company_id = ?').get(product_id, company_id).count;
  ...
  const lastOP = getDb().prepare('SELECT order_number FROM production_orders WHERE company_id = ? ORDER BY id DESC LIMIT 1').get(company_id);
  const nextNum = lastOP ? parseInt(lastOP.order_number.replace('OP-', '')) + 1 : 1;
  const order_number = `OP-${String(nextNum).padStart(4, '0')}`;

  const result = getDb().prepare('INSERT INTO production_orders (company_id, order_number, product_id, sales_order_id, customer_id, planned_quantity, planned_start_date, planned_end_date, priority, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, order_number, product_id, sales_order_id, customer_id, planned_quantity, planned_start_date, planned_end_date, priority || 'normal', notes, req.user.id);
  const opId = result.lastInsertRowid;

  // Create materials from BOM
  const boms = getDb().prepare('SELECT * FROM product_boms WHERE product_id = ? AND company_id = ?').all(product_id, company_id);
  boms.forEach(b => {
    const qty = b.quantity * planned_quantity * (1 + b.loss_percentage / 100);
    getDb().prepare('INSERT INTO production_order_materials (company_id, production_order_id, product_id, planned_quantity, unit_cost, total_cost) VALUES (?, ?, ?, ?, ?, ?)').run(company_id, opId, b.component_id, qty, b.unit_cost, qty * b.unit_cost);
  });

  // Create operations from route
  const routes = getDb().prepare('SELECT * FROM production_routes WHERE product_id = ? AND company_id = ? ORDER BY sequence').all(product_id, company_id);
  routes.forEach(r => {
    getDb().prepare('INSERT INTO production_order_operations (company_id, production_order_id, sequence, operation_name, machine_id, planned_time_minutes) VALUES (?, ?, ?, ?, ?, ?)').run(company_id, opId, r.sequence, r.operation_name, r.machine_id, r.standard_time_minutes * planned_quantity);
  });

  // Calculate planned cost
  const matCost = getDb().prepare('SELECT COALESCE(SUM(total_cost), 0) as total FROM production_order_materials WHERE production_order_id = ?').get(opId).total;
  const opCost = routes.reduce((sum, r) => sum + (r.standard_time_minutes * planned_quantity / 60 * r.hourly_cost), 0);
  getDb().prepare('UPDATE production_orders SET planned_cost = ? WHERE id = ?').run(matCost + opCost, opId);

  res.json({ id: opId, order_number });
});

app.post('/api/production-orders/:id/release', authenticateToken, (req, res) => {
  const op = getDb().prepare('SELECT * FROM production_orders WHERE id = ? AND company_id = ?').get(req.params.id, req.user.company_id);
  ...
  // Reserve materials
  const materials = getDb().prepare('SELECT * FROM production_order_materials WHERE production_order_id = ?').all(op.id);
  for (const m of materials) {
    const stock = getStockForProduct(req.user.company_id, m.product_id);
  ...
  // Reserve
  materials.forEach(m => {
    getDb().prepare('UPDATE production_order_materials SET reserved_quantity = planned_quantity, status = ? WHERE id = ?').run('reserved', m.id);
  });

  getDb().prepare('UPDATE production_orders SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run('released', op.id);
  res.json({ success: true });
});

app.post('/api/production-orders/:id/start', authenticateToken, (req, res) => {
  getDb().prepare('UPDATE production_orders SET status = ?, real_start_date = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ?').run('in_production', new Date().toISOString().split('T')[0], req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.post('/api/production-orders/:id/pause', authenticateToken, (req, res) => {
  getDb().prepare('UPDATE production_orders SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ?').run('paused', req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.post('/api/production-orders/:id/resume', authenticateToken, (req, res) => {
  getDb().prepare('UPDATE production_orders SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ?').run('in_production', req.params.id, req.user.company_id);
  res.json({ success: true });
});

app.post('/api/production-orders/:id/finish', authenticateToken, (req, res) => {
  const company_id = req.user.company_id;
  const op = getDb().prepare('SELECT * FROM production_orders WHERE id = ? AND company_id = ?').get(req.params.id, company_id);
  if (!op) return res.status(404).json({ error: 'OP não encontrada' });

  // Consume materials
  const materials = getDb().prepare('SELECT * FROM production_order_materials WHERE production_order_id = ?').all(op.id);
  materials.forEach(m => {
    getDb().prepare('UPDATE production_order_materials SET consumed_quantity = planned_quantity, status = ? WHERE id = ?').run('consumed', m.id);
    getDb().prepare('INSERT INTO stock_movements (company_id, product_id, type, quantity, unit_cost, total_cost, reason, reference_type, reference_id, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, m.product_id, 'consumo_op', m.planned_quantity, m.unit_cost, m.planned_quantity * m.unit_cost, `Consumo OP ${op.order_number}`, 'production_order', op.id, req.user.id);
  });

  // Add finished product to stock
  getDb().prepare('INSERT INTO stock_movements (company_id, product_id, type, quantity, unit_cost, total_cost, reason, reference_type, reference_id, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, op.product_id, 'producao', op.produced_quantity || op.planned_quantity, op.planned_cost / (op.planned_quantity || 1), op.planned_cost, `Produção OP ${op.order_number}`, 'production_order', op.id, req.user.id);

  getDb().prepare('UPDATE production_orders SET status = ?, real_end_date = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run('finished', new Date().toISOString().split('T')[0], op.id);
  res.json({ success: true });
});

app.post('/api/production-orders/:id/cancel', authenticateToken, (req, res) => {
  // Unreserve materials
  const materials = getDb().prepare('SELECT * FROM production_order_materials WHERE production_order_id = ?').all(req.params.id);
  materials.forEach(m => {
    getDb().prepare('UPDATE production_order_materials SET reserved_quantity = 0, status = ? WHERE id = ?').run('cancelled', m.id);
  });
  getDb().prepare('UPDATE production_orders SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ?').run('cancelled', req.params.id, req.user.company_id);
  res.json({ success: true });
});

// ==================== PRODUCTION LOGS ====================
app.get('/api/production-logs', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const logs = getDb().prepare('SELECT pl.*, po.order_number, p.name as product_name, u.name as user_name, m.name as machine_name, poo.operation_name FROM production_logs pl JOIN production_orders po ON pl.production_order_id = po.id JOIN products p ON po.product_id = p.id LEFT JOIN users u ON pl.user_id = u.id LEFT JOIN machines m ON pl.machine_id = m.id LEFT JOIN production_order_operations poo ON pl.operation_id = poo.id WHERE pl.company_id = ? ORDER BY pl.created_at DESC LIMIT 200').all(filter.company_id);
  res.json(logs);
});

app.get('/api/production-logs/order/:productionOrderId', authenticateToken, (req, res) => {
  const logs = getDb().prepare('SELECT pl.*, u.name as user_name, m.name as machine_name, poo.operation_name FROM production_logs pl LEFT JOIN users u ON pl.user_id = u.id LEFT JOIN machines m ON pl.machine_id = m.id LEFT JOIN production_order_operations poo ON pl.operation_id = poo.id WHERE pl.production_order_id = ? AND pl.company_id = ? ORDER BY pl.created_at DESC').all(req.params.productionOrderId, req.user.company_id);
  res.json(logs);
});

app.post('/api/production-logs', authenticateToken, (req, res) => {
  const company_id = req.user.company_id;
  ...
  const result = getDb().prepare('INSERT INTO production_logs (company_id, production_order_id, operation_id, user_id, machine_id, start_time, end_time, total_time_minutes, produced_quantity, rejected_quantity, stop_reason, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(company_id, production_order_id, operation_id, req.user.id, machine_id, start_time, end_time, total_time_minutes, produced_quantity || 0, rejected_quantity || 0, stop_reason, notes);

  // Update OP quantities
  if (produced_quantity) {
    getDb().prepare('UPDATE production_orders SET produced_quantity = produced_quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(produced_quantity, production_order_id);
  }
  if (rejected_quantity) {
    getDb().prepare('UPDATE production_orders SET rejected_quantity = rejected_quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(rejected_quantity, production_order_id);
  }

  // Update operation time
  if (operation_id && total_time_minutes > 0) {
    getDb().prepare('UPDATE production_order_operations SET real_time_minutes = real_time_minutes + ? WHERE id = ?').run(total_time_minutes, operation_id);
  }

  // Calculate real cost
  const op = getDb().prepare('SELECT * FROM production_orders WHERE id = ?').get(production_order_id);
  const logs = getDb().prepare('SELECT * FROM production_logs WHERE production_order_id = ?').all(production_order_id);
  const machineCost = logs.reduce((sum, l) => {
    const machine = l.machine_id ? getDb().prepare('SELECT hourly_cost FROM machines WHERE id = ?').get(l.machine_id) : null;
    return sum + (machine ? l.total_time_minutes / 60 * machine.hourly_cost : 0);
  }, 0);
  const matCost = getDb().prepare('SELECT COALESCE(SUM(total_cost), 0) as total FROM production_order_materials WHERE production_order_id = ?').get(production_order_id).total;
  getDb().prepare('UPDATE production_orders SET real_cost = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(matCost + machineCost, production_order_id);

  res.json({ id: result.lastInsertRowid });
});

// ==================== PLANNING ====================
app.get('/api/planning', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const orders = getDb().prepare(`SELECT so.*, c.name as customer_name FROM sales_orders so JOIN customers c ON so.customer_id = c.id WHERE so.company_id = ? AND so.status IN ('open', 'in_planning') ORDER BY so.delivery_date ASC`).all(filter.company_id);
  const result = orders.map(o => {
    const items = getDb().prepare(`SELECT soi.*, p.name as product_name, p.code as product_code, p.type as product_type FROM sales_order_items soi JOIN products p ON soi.product_id = p.id WHERE soi.sales_order_id = ?`).all(o.id);
    const enrichedItems = items.map(item => {
      const stock = getStockForProduct(filter.company_id, item.product_id);
      const bom = getDb().prepare('SELECT COUNT(*) as count FROM product_boms WHERE product_id = ?').get(item.product_id);
...
app.post('/api/planning/check-materials', authenticateToken, (req, res) => {
  const company_id = req.user.company_id;
  const { product_id, quantity } = req.body;
  const boms = getDb().prepare('SELECT pb.*, p.name as component_name FROM product_boms pb JOIN products p ON pb.component_id = p.id WHERE pb.product_id = ?').all(product_id);
  const check = boms.map(b => {
    const needed = b.quantity * quantity * (1 + b.loss_percentage / 100);
    const stock = getStockForProduct(company_id, b.component_id);
...
app.get('/api/dashboard/summary', authenticateToken, (req, res) => {
  ...
  const openOps = getDb().prepare('SELECT COUNT(*) as count FROM production_orders WHERE company_id = ? AND status IN (?, ?, ?, ?)').get(cid, 'planned', 'released', 'in_production', 'paused').count;
  const delayedOps = getDb().prepare("SELECT COUNT(*) as count FROM production_orders WHERE company_id = ? AND status IN ('in_production', 'released') AND planned_end_date < date('now')").get(cid).count;
  const finishedThisMonth = getDb().prepare("SELECT COUNT(*) as count FROM production_orders WHERE company_id = ? AND status = 'finished' AND strftime('%Y-%m', real_end_date) = strftime('%Y-%m', 'now')").get(cid).count;
  const todayProduction = getDb().prepare("SELECT COALESCE(SUM(produced_quantity), 0) as total FROM production_logs WHERE company_id = ? AND date(start_time) = date('now')").get(cid).total;
  const criticalStock = getDb().prepare('SELECT COUNT(*) as count FROM products WHERE company_id = ? AND active = 1').all(cid).filter(p => getStockForProduct(cid, p.id) <= p.min_stock).length;
  const plannedCostMonth = getDb().prepare("SELECT COALESCE(SUM(planned_cost), 0) as total FROM production_orders WHERE company_id = ? AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')").get(cid).total;
  const realCostMonth = getDb().prepare("SELECT COALESCE(SUM(real_cost), 0) as total FROM production_orders WHERE company_id = ? AND status = 'finished' AND strftime('%Y-%m', real_end_date) = strftime('%Y-%m', 'now')").get(cid).total;
  const totalScrap = getDb().prepare("SELECT COALESCE(SUM(rejected_quantity), 0) as total FROM production_orders WHERE company_id = ? AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')").get(cid).total;
...
app.get('/api/dashboard/charts', authenticateToken, (req, res) => {
  ...
  const dailyProd = getDb().prepare("SELECT date(start_time) as day, COALESCE(SUM(produced_quantity), 0) as total FROM production_logs WHERE company_id = ? AND start_time >= date('now', '-7 days') GROUP BY date(start_time) ORDER BY day").all(cid);

  // OP status distribution
  const statusDist = getDb().prepare('SELECT status, COUNT(*) as count FROM production_orders WHERE company_id = ? GROUP BY status').all(cid);

  // Top products
  const topProducts = getDb().prepare('SELECT p.name, SUM(po.planned_quantity) as total FROM production_orders po JOIN products p ON po.product_id = p.id WHERE po.company_id = ? GROUP BY po.product_id ORDER BY total DESC LIMIT 5').all(cid);
...
app.get('/api/dashboard/alerts', authenticateToken, (req, res) => {
  ...
  // Low stock
  const products = getDb().prepare('SELECT * FROM products WHERE company_id = ? AND active = 1').all(cid);
  products.forEach(p => {
    const stock = getStockForProduct(cid, p.id);
  ...
  // Delayed OPs
  const delayedOps = getDb().prepare("SELECT order_number, p.name as product_name, planned_end_date FROM production_orders po JOIN products p ON po.product_id = p.id WHERE po.company_id = ? AND po.status IN ('in_production', 'released') AND po.planned_end_date < date('now')").all(cid);
  ...
  const noBom = getDb().prepare("SELECT p.name FROM products p WHERE p.company_id = ? AND p.type = 'finished' AND p.active = 1 AND NOT EXISTS (SELECT 1 FROM product_boms WHERE product_id = p.id)").all(cid);
...
app.get('/api/subscriptions', authenticateToken, (req, res) => {
  const filter = companyFilter(req);
  const subs = getDb().prepare('SELECT s.*, c.name as company_name FROM subscriptions s JOIN companies c ON s.company_id = c.id WHERE s.company_id = ?').all(filter.company_id);
  res.json(subs);
});

app.put('/api/subscriptions/:id', authenticateToken, requireRole('super_admin', 'admin'), (req, res) => {
  const { plan, status, monthly_value, next_billing_date } = req.body;
  getDb().prepare('UPDATE subscriptions SET plan=?, status=?, monthly_value=?, next_billing_date=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(plan, status, monthly_value, next_billing_date, req.params.id);
  res.json({ success: true });
});

// ==================== AUDIT LOGS ====================
function logAudit(company_id, user_id, action, entity, entity_id, old_value, new_value) {
  getDb().prepare('INSERT INTO audit_logs (company_id, user_id, action, entity, entity_id, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?, ?)').run(company_id, user_id, action, entity, entity_id, old_value ? JSON.stringify(old_value) : null, new_value ? JSON.stringify(new_value) : null);
}
...
async function startServer() {
  ...
  // Auto-seed if empty
  const companyCount = getDb().prepare('SELECT COUNT(*) as count FROM companies').get().count;
  if (companyCount === 0) {
    ...
    // Super Admin company
    db.prepare("INSERT INTO companies (id, name, trade_name, cnpj, email, phone, plan, status) VALUES (1, 'PCP Pro Industrial', 'PCP Pro', '00.000.000/0001-00', 'admin@pcppro.com', '11999990000', 'industrial', 'active')").run();
